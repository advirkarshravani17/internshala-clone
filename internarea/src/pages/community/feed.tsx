...
import { Heart, MessageCircle, Share2, Search as SearchIcon } from "lucide-react";
...
const Feed: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      try {
        const res = await axios.get<PostFromBackend[]>("https://internshala-clone-2qo8.onrender.com/api/posts");
  ...
  const handleLike = async (id: string) => {
    ...
    const res = await axios.put(
      `https://internshala-clone-2qo8.onrender.com/api/posts/like/${id}`,
      { userId }
    );
  ...
  const handleShare = async (id: string) => {
    try {
      await axios.put<{ message: string }>(`https://internshala-clone-2qo8.onrender.com/api/posts/share/${id}`);
      setPosts(posts.map((post) => (post.postId === id ? { ...post, shares: post.shares + 1 } : post)));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleCommentPost = async (id: string) => {
    ...
    try {
      const res = await axios.post<{ comments: Comment[] }>(
        `https://internshala-clone-2qo8.onrender.com/api/posts/comment/${id}`,
        {
          user: "You",
      ...
      setPosts(
        posts.map((p) =>
          p.postId === id ? { ...p, comments: res.data.comments, newComment: "" } : p
        )
      );
    } catch (err) {
      console.error(err);
    ...
    <div className="min-h-screen bg-gray-100 py-6">
      <div className="max-w-6xl mx-auto flex gap-6">
        ...
        <div className="flex-1 space-y-6">
          ...
          {posts.map((post) => (
            <div key={post.postId} className="bg-white rounded-xl shadow-md p-4">
              {/* User Info */}
              <div
              ...
              {/* Post Image */}
              {post.postImage && <img src={post.postImage} alt="post" className="w-full rounded-lg mt-4" />}
              ...
              <div className="flex justify-between mt-4">
                <button
                  ...
                  className={`flex items-center gap-1 transition ${post.liked ? "text-red-500" : "text-gray-600"
                    }`}
                >
                ...
                <div className="mt-2 flex gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setPosts(
                        posts.map((p) =>
                          p.postId === post.postId ? { ...p, newComment: e.target.value } : p
                        )
                      )
                    }
